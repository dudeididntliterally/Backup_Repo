...
do
    ...
    function utility:InitDragging(frame,button)
        ...
        local function getInputPosition(input) return Vector2.new(input.Position.X, input.Position.Y) end
        local inputBegan = button.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1
                or input.UserInputType == Enum.UserInputType.Touch then
                _dragging = true
                _dragging_offset = getInputPosition(input) - frame.AbsolutePosition
            end
        end)
        ...
        local updateEvent
        LPH_JIT_MAX(function()
            updateEvent = UIS.InputChanged:Connect(function(input)
                ...
                if input.UserInputType == Enum.UserInputType.MouseMovement
                    ...
                    frame.Position = UDim2.fromOffset(
                        pos.X - _dragging_offset.X + (frame.AbsoluteSize.X * frame.AnchorPoint.X),
                        pos.Y - _dragging_offset.Y + (frame.AbsoluteSize.Y * frame.AnchorPoint.Y)
                    )
                end
            end)
        end)()
