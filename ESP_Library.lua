if getgenv().playeresp_singleton and getgenv().playeresp_singleton.active then return getgenv().playeresp_singleton end
...
local function untrack_player(p) clear_player(p) end
if not esp.connections.playeradded then esp.connections.playeradded = players.PlayerAdded:Connect(track_player) end
if not esp.connections.playerremoving then esp.connections.playerremoving = players.PlayerRemoving:Connect(untrack_player) end
if not esp.render_connection then
	esp.render_connection = runservice.RenderStepped:Connect(function()
		if not esp.enabled and not esp.part_enabled then return end
...
for _, p in ipairs(players:GetPlayers()) do track_player(p) end
