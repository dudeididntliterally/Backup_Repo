...
do
    ...
    function utility:InitDragging(frame, button)
        ...
        local logs = {}
        local function addLog(str) table.insert(logs, str) end
        local inset = GuiService:GetGuiInset()
        addLog(string.format("GuiInset=(%.1f,%.1f)", inset.X, inset.Y))
        addLog(string.format("button ClassName: %s | Active: %s", button.ClassName, tostring(button.Active)))
        ...
        local inputBegan = button.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1
                ...
                _dragging_offset = pos - frame.AbsolutePosition
                addLog(string.format(
                    "DRAG START | input=(%.1f,%.1f) | frame.AbsolutePosition=(%.1f,%.1f) | offset=(%.1f,%.1f)",
                    pos.X, pos.Y, frame.AbsolutePosition.X, frame.AbsolutePosition.Y,
                    _dragging_offset.X, _dragging_offset.Y
                ))
            end
        end)
        ...
        local updateEvent = UIS.InputChanged:Connect(function(input)
            if not _dragging or not _dragging_offset then return end
            if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
                local pos = Vector2.new(input.Position.X, input.Position.Y)
                local newX = pos.X - _dragging_offset.X + (frame.AbsoluteSize.X * frame.AnchorPoint.X)
                local newY = pos.Y - _dragging_offset.Y + (frame.AbsoluteSize.Y * frame.AnchorPoint.Y)
                frame.Position = UDim2.fromOffset(newX, newY)
                frameCount = frameCount + 1
                if frameCount <= 5 then
                    addLog(string.format(
                        "frame %d | input=(%.1f,%.1f) | computed new pos=(%.1f,%.1f) | AbsPos AFTER set=(%.1f,%.1f)",
                        frameCount, pos.X, pos.Y, newX, newY,
