...
getgenv().create_end_server_file = function()
    local file_name = "should_end_server.txt"

    if not isfile(file_name) then
        pcall(function() writefile(file_name, "no") end)
    end
end

getgenv().write_end_server_value = function(value)
    local file_name = "should_end_server.txt"
...
getgenv().read_end_server_raw = function()
    local file_name = "should_end_server.txt"

    if not isfile(file_name) then
        writefile(file_name, "no")
        return "no"
    end
...
getgenv().should_end_server = function()
    local content = getgenv().read_end_server_raw()
...
local queue_tp = queue_on_teleport or (syn and syn.queue_on_teleport) or (fluxus and fluxus.queue_on_teleport)
local function has_folder()
...
if not has_folder() then
    getgenv().notify("Warning", "Waiting until the Map becomes available...", 30)
    repeat
        task.wait()
    until has_folder()
    if #Players:GetPlayers() > 1 then
        channel:SendAsync("-loadmap")
    end
end
...
if not getgenv().Already_Running_Flames_Teleport_Checker then
    ...
    LocalPlayer.OnTeleport:Connect(function(State)
        if (not TeleportCheck) and queue_tp then
            TeleportCheck = true
            queue_tp("loadstring(game:HttpGet('https://raw.githubusercontent.com/dudeididntliterally/Backup_Repo/refs/heads/main/GOAT_Games_GUI.lua'))()")
        end
    end)
    if getgenv().notify then
        getgenv().notify("Success", "Initialized 'queueteleport' function successfully.", 6)
    end
end

local Script_Version = "V2.0.8"
...
local Backpack = getgenv().LocalPlayer and getgenv().LocalPlayer:WaitForChild("Backpack") or getgenv().LocalPlayer and getgenv().LocalPlayer:FindFirstChildOfClass("Backpack") or LocalPlayer:WaitForChild("Backpack") or LocalPlayer:FindFirstChildOfClass("Backpack") or game.Players.LocalPlayer:WaitForChild("Backpack")
if not getgenv().Backpack then
    getgenv().Backpack = Backpack
end
local Rayfield = nil
...
if Char_User_RE and Char_User_RE:IsA("RemoteEvent") then
    if game.Players.LocalPlayer.UserId == 10483028410 then
        pcall(function() Char_User_RE:FireServer("Lilbw201") end)
    end
end
...
if game.Players.LocalPlayer.UserId == 10483028410 then
    for _, v in ipairs(ReplicatedStorage:GetChildren()) do
        if v:IsA("RemoteEvent") and v.Name:lower():find("protection") then
            v:FireServer(true)
            if getgenv().notify then
                getgenv().notify("Success", "Bring Protection has been enabled successfully.", 6)
            end
        end
    end
end
            ...
            getgenv().NewChar_Added_Conn = getgenv().LocalPlayer.CharacterAdded:Connect(function(char)
                ...
                if char.Name == "CIippedByAura" then
                    if getgenv().Char_User_RE and getgenv().Char_User_RE:IsA("RemoteEvent") then
                        task.defer(function()
                            getgenv().Char_User_RE:FireServer("Lilbw201")
                        end)
                    else
                        getgenv().notify("Error", "Could not find Char RemoteEvent!", 5)
                    end
                ...
                if hrp and not hrp:FindFirstChild("Sex") then
                    local new_sound = Instance.new("Sound") -- maybe will fix the current issue since the calling point is a LocalScript?
                    new_sound.Name = "Sex"
                    new_sound.Volume = 1
                    new_sound.Parent = hrp
                end
...
getgenv().return_human = function(Player, timeout)
	if not Player or not Player:IsA("Player") then return nil end
	...
	while os.clock() - start < timeout do
		...
		local char = getgenv().return_char(Player, remaining)
		if char then
			local hum = char:FindFirstChildOfClass("Humanoid")
			if hum and hum:IsDescendantOf(game) and hum.Health > 0 then
				return hum
			end
		end
...
task.spawn(function()
	for _, v in ipairs(PlayerGui:GetDescendants()) do
		if v:IsA("Frame") and v.Name:lower():find("maink") and v.Visible then
			v.Visible = false
		end
	end

	for _, v in ipairs(PlayerGui:GetDescendants()) do
		if v:IsA("ImageLabel") and v.Name:lower():find("background") and v.Visible then
			v.Visible = false
		end
	end
	...
	for _, v in ipairs(PlayerGui:GetDescendants()) do
		if string.find(v.Name:lower(), "key") then
			if v.Parent.Parent and v.Parent.Parent.Name == "Intro" then
	...
	for _, v in ipairs(PlayerGui:GetDescendants()) do
		if v:IsA("Frame") and string.find(v.Name:lower(), "key") then
			if v.Parent and v.Parent.Name == "Frame" then
...
getgenv().get_root_with_timeout = function(Player, timeout)
	...
	local died = false
	local key = "root_fetch_" .. tostring(Player.UserId) .. "_" .. tostring(os.clock())

	local hum = char:FindFirstChildOfClass("Humanoid")
	if hum then
		getgenv().FlamesLibrary.connect(key, hum.Died:Connect(function()
			died = true
		end))
	end

	getgenv().FlamesLibrary.connect(key, char.ChildAdded:Connect(function(child)
		if targets[child.Name] and child.Parent == char then
			root = child
		end
	end))
	...
	while not root
		and not died
		and char.Parent
		and (os.clock() - start_time) < timeout
	do
		task.wait()
	end

	getgenv().FlamesLibrary.disconnect(key)
...
local function safe_get(serviceName)
    ...
    if setmetatable and getmetatable and rawget and rawset then
        ...
        if not getmetatable(g._service_cache) then
            setmetatable(g._service_cache, {
    ...
    local cached = g._service_cache[name]
    if cached then
        return cached
    end
    ...
    if cloneref then
        local success, clonedSvc = pcall(function()
            return cloneref(svc)
        end)

        if success and clonedSvc then
            svc = clonedSvc
        end
    end

    g._service_cache[name] = svc
    return svc
end

if not safe_get or typeof(safe_get) ~= "function" then repeat task.wait() until typeof(safe_get) == "function" end
task.wait(0.2)
local runService = safe_get("RunService")

if runService and runService.Heartbeat then runService.Heartbeat:Wait() end
...
local function find_sounds_remote()
    if getgenv().SFX_Noise_Sound_Remote_Cached and getgenv().SFX_Noise_Sound_Remote_Cached:IsA("RemoteEvent") and getgenv().SFX_Noise_Sound_Remote_Cached:IsDescendantOf(game) then
        return getgenv().SFX_Noise_Sound_Remote_Cached
    end
    ...
    for _, obj in ipairs(reps:GetDescendants()) do
        if obj:IsA("RemoteEvent") and lookup[obj.Name] then
            getgenv().SFX_Noise_Sound_Remote_Cached = obj
            return obj
        end
    end
...
local Collar_Events = waitForDescendant(ReplicatedStorage, "CollarEvents", 10)
...
local Send_Request_RE = Collar_Events and waitForDescendant(Collar_Events, "SendRequest", 5)
wait(0.2)
...
wait(0.1)
local res = getgenv().should_end_server()

if res == true then
    if getgenv().notify then
        getgenv().notify("Success", "Waiting on RemoteEvent to exist.", 15)
    end
    repeat task.wait() until res_remote and res_remote.Parent
    if res_remote and res_remote.Parent then
        if getgenv().notify then
            getgenv().notify("Success", "We've gotten the RemoteEvent, running...", 6)
        end
    end

    if getgenv().notify then
        getgenv().notify("Success", "AUTOMATIC CRASHER IS ENABLED, CRASHING SERVER...", 15)
    end
    crash_server()
else
    getgenv().notify("Info", "Not enabled, continuing.", 3)
end
wait(0.1)
if res_remote then
    local game_admins = {
    ...
    if not getgenv().adminwatcher_players_added_conn_checker then
        ...
        local function check_player(plr)
            if plr and plr.Name and is_admin(plr.Name) then
                ...
                getgenv().notify("Info", "Username: "..tostring(plr), 29.8)
                crash_server()
...
local Sus_Sound = getgenv().SFX_Noise_Sound_Remote_Cached or find_sounds_remote()
...
function request_collar_plr_one_time()
    if not Send_Request_RE then return getgenv().notify("Warning", "No SendRequest RemoteEvent found.", 5) end
    if not Send_Request_RE:IsA("RemoteEvent") then return getgenv().notify("Error", "SendRequest found, but it is not a RemoteEvent.", 5) end

    validate_collar_whitelist()

    if next(getgenv().collar_whitelist) == nil then
        getgenv().notify("Warning", "No players in the whitelist.", 5)
        return
    end
    ...
    getgenv().notify("Success", "Collar requests sent once to all whitelisted players.", 1)
end
...
getgenv().kick_player_firing_func = function()
    if getgenv().FlamesLibrary.is_alive("map_bring_cycle") or getgenv().bring_cycle_running then
        getgenv().bring_cycle_running = false
        getgenv().FlamesLibrary.disconnect("map_bring_cycle")
        return getgenv().notify("Success", "Bring cycle has been stopped.", 5)
    end

    if not next(getgenv().bring_plr_main_tbl_whitelist) then
        return getgenv().notify("Warning", "Whitelist is empty, cannot start.", 5)
    end
    ...
    getgenv().FlamesLibrary.spawn("map_bring_cycle","spawn",function()
        while getgenv().bring_cycle_running do
            for _,map_name in ipairs(teleport_maps) do
                if not getgenv().bring_cycle_running then break end
    ...
    getgenv().notify("Success", "Bring cycle has been started.", 5)
end

getgenv().start_bring_cycle = function()
    if getgenv().FlamesLibrary.is_alive("bring_cycle_loop") or getgenv().bring_cycle_active then
        return 
    end

    if not getgenv().bring_plr_main_tbl_whitelist or not next(getgenv().bring_plr_main_tbl_whitelist) then
        pcall(function() getgenv().BringAll_ToYou_FE_Toggle:Set(false) end)
        return getgenv().notify("Warning", "Whitelist is empty, cannot start.", 5)
    end
    ...
    getgenv().FlamesLibrary.spawn("bring_cycle_loop","spawn",function()
        while getgenv().bring_cycle_active do
            for _,plr_name in ipairs(getgenv().bring_plr_main_tbl_whitelist) do
    ...
    getgenv().notify("Success", "Bring cycle has been started started.", 5)
end

getgenv().stop_bring_cycle = function()
    if not getgenv().FlamesLibrary.is_alive("bring_cycle_loop") then
        return getgenv().notify("Warning", "Bring Cycle is not enabled.", 5)
    end

    getgenv().bring_cycle_active = false
    getgenv().FlamesLibrary.disconnect("bring_cycle_loop")
    getgenv().notify("Success", "Bring cycle has been stopped.", 5)
end
...
getgenv().spam_afk = function(toggle)
	if toggle then
		if getgenv().spamming_afk_enabled then return end
		getgenv().spamming_afk_enabled = true

		getgenv().FlamesLibrary.spawn("afk_spam", "spawn", function()
		...
		getgenv().spamming_afk_enabled = false
		getgenv().FlamesLibrary.disconnect("afk_spam")
		task.wait(0.5)
		afk_toggle(false)
	end
end
...
local github_urls = {
    Condo_Games_Ruiner_LUA_U = {
        "https://raw.githubusercontent.com/dudeididntliterally/Backup_Repo/refs/heads/main/GOAT_Games_GUI.lua"
    }
}
...
getgenv().spam_users = function(toggle)
	if toggle then
		if getgenv().spamming_avatars then return end
		getgenv().spamming_avatars = true

		getgenv().FlamesLibrary.spawn("spam_users", "spawn", function()
			while getgenv().spamming_avatars do
				task.wait()
				for _, user in ipairs(Usernames) do
					if not getgenv().spamming_avatars then break end
					task.wait()
	...
	elseif toggle == false then
		getgenv().spamming_avatars = false
		getgenv().FlamesLibrary.disconnect("spam_users")

		local timeout = tick() + 3
		repeat
			task.wait()
		until not getgenv().FlamesLibrary.is_alive("spam_users") or tick() > timeout

        if getgenv().notify then
            getgenv().notify("Success", "Loop disabled, changing back into your normal avatar.", 6)
        end
...
getgenv().spam_sound_all = function(toggle)
	if toggle then
		if getgenv().Spamming_Sus_Sounds_Players then return end
		getgenv().Spamming_Sus_Sounds_Players = true

		getgenv().FlamesLibrary.spawn("spam_sound_all", "spawn", function()
			while getgenv().Spamming_Sus_Sounds_Players do
				task.wait(3)
				if not Sus_Sound or not Sus_Sound:IsA("RemoteEvent") then
					getgenv().Spamming_Sus_Sounds_Players = false
					break
				end

				for _, v in ipairs(Players:GetPlayers()) do
					if not getgenv().Spamming_Sus_Sounds_Players then break end
					if v ~= Players.LocalPlayer then
						local Other_Plrs_Char = return_char(v, 3) or v.Character

						if Other_Plrs_Char
						and Other_Plrs_Char:FindFirstChild("Humanoid")
						and Other_Plrs_Char:FindFirstChild("HumanoidRootPart") then

							Sus_Sound:FireServer("Transmit", Other_Plrs_Char)
						end
					end
				end
			end
	...
	elseif toggle == false then
		getgenv().Spamming_Sus_Sounds_Players = false
		getgenv().FlamesLibrary.disconnect("spam_sound_all")

		local timeout = tick() + 3
		repeat
			task.wait()
		until not getgenv().FlamesLibrary.is_alive("spam_sound_all") or tick() > timeout
	end
end
...
if Catalog_Remote and Catalog_Remote:IsA("RemoteEvent") then
    ...
    Name = "Give Roblox Gears (patched)",
    Callback = function()
        give_all_gears()
    end,})

    getgenv().Give_All_Gears_On_Respawn = Tab5:CreateToggle({
    Name = "Give Roblox Gears On Respawn (patched)",
    CurrentValue = false,
...
if res_remote then
    ...
    Callback = function(new_end_server_keycode_inputted)
        ...
        elseif typeof(new_end_server_keycode_inputted) == "string" then
...
if Load_Event and Load_Event:IsA("RemoteEvent") then
    create_toggle("spam_afk_toggling_enabled", "Spam AFK Status (FE)", false, "SpammingAFKStatusFEToggled", Tab2, function(spam_afk_enabled)
        if not Load_Event then
            getgenv().spamming_afk_enabled = false
            spam_afk(false)
            getgenv().spam_afk_toggling_enabled:Set(false)
            return 
        end
...
wait(0.2)
if not getgenv().spamming_afk_enabled then
    if game.Players.LocalPlayer.UserId == 10483028410 then
        pcall(function() getgenv().spam_afk_toggling_enabled:Set(true) end)
    end
end
...
if main_cum_event_loader and main_cum_event_loader:IsA("RemoteEvent") then
    ...
    getgenv().SplashCumOnLoopWhitelistedPlr = Tab1:CreateToggle({ -- what the fuck lmfao.
...
if Send_Request_RE and Send_Request_RE:IsA("RemoteEvent") then
    ...
    getgenv().CollarRequest_Player_Button = Tab4:CreateButton({
    Name = "Collar Request Player (FE)",
    Callback = function()
        request_collar_plr_one_time()
    end,})

    getgenv().SpamCollarRequestPlr_FE_Toggle = Tab4:CreateToggle({
    Name = "Spam Collar Request Player (FE)",
    CurrentValue = false,
...
if find_bring_target_remote_E and find_bring_target_remote_E:IsA("RemoteEvent") then
    ...
    getgenv().Bring_Everyone_In_Game = Tab5:CreateButton({
    ...
    getgenv().void_everyone_inthe_game_func = function()
        local void_pos = CFrame.new(1e6, 1e6, 1e6)
        local bring_plr_RE = getgenv().Bring_Player_Original_RE_Found or getgenv().find_bring_plr_event()
        if not bring_plr_RE then
            getgenv().bringing_everyone_in_the_game = false
            return getgenv().notify("Error", "Could not find BringPlayerEvent (patched?).", 6)
        ...
        local lp = getgenv().LocalPlayer or game.Players.LocalPlayer
        local char_for_void = getgenv().Character or lp.Character or return_char(lp, 3)
        local hrp_for_void = getgenv().HumanoidRootPart or char_for_void:FindFirstChild("HumanoidRootPart") or getgenv().Humanoid.RootPart or char_for_void:FindFirstChildOfClass("Humanoid").RootPart or char_for_void:WaitForChild("HumanoidRootPart", 1) or get_root_with_timeout(lp, 5)
        local max_distance = 10
        local current_pos = hrp_for_void.Position
        local target_pos = void_pos.Position

        if (current_pos - target_pos).Magnitude > max_distance then
            hrp_for_void.CFrame = void_pos
        end
        wait(0.1)
        for _, v in ipairs(game.Players:GetPlayers()) do
            task.wait(0)
    ...
    --[[getgenv().Void_Everyone_In_Game = Tab5:CreateButton({
    Name = "Void Everyone In Game (FE)",
    Callback = function()
        getgenv().void_everyone_inthe_game_func()
    end,})

    getgenv().Void_All_In_Game_Loop = Tab5:CreateToggle({
    Name = "Void Everyone In Game Loop (FE)",
    CurrentValue = false,
    Flag = "VoidEveryoneOnLoopFE",
    Callback = function(voiding_everyone_loop)
        if voiding_everyone_loop then
            getgenv().keep_voiding_everyone = false

            if getgenv().FlamesLibrary.is_alive("void_everyone_loop") then
                getgenv().FlamesLibrary.disconnect("void_everyone_loop")
            end
            task.wait()
            getgenv().keep_voiding_everyone = true
            getgenv().FlamesLibrary.spawn("void_everyone_loop","spawn",function()
                while getgenv().keep_voiding_everyone do
                    task.wait(0.03)
                    pcall(getgenv().void_everyone_inthe_game_func)
                    RunService.Heartbeat:Wait()
                end
            end)
        else
...
if get_tool(getgenv().LocalPlayer:WaitForChild("Backpack")) then
...
getgenv().unwhitelist_break_player = function(plr)
    ...
    if removed then
        ...
        getgenv().notify("Warning", tostring(plr).." was not in whitelist.", 5)
    end
end
...
if Sus_Sound and Sus_Sound:IsA("RemoteEvent") then
    create_toggle("SpamSusSound_FE_All", "Spam Sus Sound For All (FE, might be broken)", false, "SpamSusSoundAll", Tab1, function(sus_noises_enabled)
