...
do
    ...
    function utility:DoClickEffect(element)
        ...
        if corner then corner:Clone().Parent = effect end
    ...
    function utility:InitDragging(frame, button)
        ...
        local logs = {}
        local function add_log(str) table.insert(logs, str) end

        local inset = GuiService:GetGuiInset()
        local screen_gui = frame
        while screen_gui and not screen_gui:IsA("ScreenGui") do
            screen_gui = screen_gui.Parent
        end

        add_log(string.format("GuiInset=(%.1f,%.1f)", inset.X, inset.Y))
        add_log(string.format(
            "ScreenGui=%s | Parent=%s | IgnoreGuiInset=%s",
            screen_gui and screen_gui.Name or "nil",
            screen_gui and tostring(screen_gui.Parent) or "nil",
            screen_gui and tostring(screen_gui.IgnoreGuiInset) or "nil"
        ))
        add_log(string.format(
            "Frame Name=%s | ClassName=%s | AnchorPoint=(%.2f,%.2f)",
            frame.Name, frame.ClassName, frame.AnchorPoint.X, frame.AnchorPoint.Y
        ))
        add_log(string.format(
            "Frame Position=%s | AbsolutePosition=(%.1f,%.1f) | AbsoluteSize=(%.1f,%.1f)",
            tostring(frame.Position), frame.AbsolutePosition.X, frame.AbsolutePosition.Y,
            frame.AbsoluteSize.X, frame.AbsoluteSize.Y
        ))
        add_log(string.format(
            "Button ClassName=%s | Name=%s | Active=%s",
            button.ClassName, button.Name, tostring(button.Active)
        ))

        local is_dragging = false
        local dragging_offset
        local frame_count = 0
        ...
        local input_began = button.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1
                ...
                dragging_offset = pos - frame.AbsolutePosition
                add_log(string.format(
                    "DRAG START | InputType=%s | input=(%.1f,%.1f) | frame.AbsolutePosition=(%.1f,%.1f) | frame.Position=%s | AnchorPoint=(%.2f,%.2f) | AbsoluteSize=(%.1f,%.1f) | offset=(%.1f,%.1f)",
                    input.UserInputType.Name, pos.X, pos.Y,
                    frame.AbsolutePosition.X, frame.AbsolutePosition.Y,
                    tostring(frame.Position),
                    frame.AnchorPoint.X, frame.AnchorPoint.Y,
                    frame.AbsoluteSize.X, frame.AbsoluteSize.Y,
                    dragging_offset.X, dragging_offset.Y
                ))
            end
        end)
        ...
        local input_ended = UIS.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1
                ...
                is_dragging = false
                add_log(string.format(
                    "DRAG END | frame.AbsolutePosition=(%.1f,%.1f) | frame.Position=%s",
                    frame.AbsolutePosition.X, frame.AbsolutePosition.Y, tostring(frame.Position)
                ))
                dragging_offset = nil
        ...
        local update_event = UIS.InputChanged:Connect(function(input)
            if not is_dragging or not dragging_offset then return end
            if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
                local pos = Vector2.new(input.Position.X, input.Position.Y)
                local new_x = pos.X - dragging_offset.X + (frame.AbsoluteSize.X * frame.AnchorPoint.X)
                local new_y = pos.Y - dragging_offset.Y + (frame.AbsoluteSize.Y * frame.AnchorPoint.Y)
                local pre_set_abs_pos = frame.AbsolutePosition
                frame.Position = UDim2.fromOffset(new_x, new_y)
                frame_count = frame_count + 1
                if frame_count <= 20 then
                    add_log(string.format(
                        "frame %d | InputType=%s | input=(%.1f,%.1f) | pre-set AbsPos=(%.1f,%.1f) | computed new pos=(%.1f,%.1f) | AbsPos AFTER set=(%.1f,%.1f) | AbsoluteSize=(%.1f,%.1f) | drift=(%.1f,%.1f)",
                        frame_count, input.UserInputType.Name, pos.X, pos.Y,
                        pre_set_abs_pos.X, pre_set_abs_pos.Y,
                        new_x, new_y,
                        frame.AbsolutePosition.X, frame.AbsolutePosition.Y,
                        frame.AbsoluteSize.X, frame.AbsoluteSize.Y,
                        frame.AbsolutePosition.X - new_x, frame.AbsolutePosition.Y - new_y
                    ))
                end
            end
        end)
