...
local function GetIcon(icon, source)
	...
	elseif source == "Lucide" then
		...
		if icon ~= nil and IconModule[source] then
			...
			local result = sourceicon[icon]
			if not result then
				warn("GetIcon | No icon named '" .. tostring(icon) .. "' found in IconModule." .. tostring(source))
			end
			return result
		else
			warn("GetIcon | IconModule has no source table named '" .. tostring(source) .. "'")
			return nil
		end
	end
end
...
local function BuildConfigPath(ConfigurationFolder, hasRoot, ConfigFolder, fileName)
	if hasRoot then
		return ConfigurationFolder .. "/" .. hasRoot .. "/" .. ConfigFolder .. "/" .. fileName
	else
		return ConfigurationFolder .. "/" .. ConfigFolder .. "/" .. fileName
	end
end
	...
	for FlagName, Flag in pairs(Luna.Flags) do
		...
		if FlagValue ~= nil then
			if Flag.Type == "ColorPicker" then
				...
				local CurrentVal = Flag.CurrentValue
				if CurrentVal == nil then CurrentVal = Flag.CurrentKeybind end
				if CurrentVal == nil then CurrentVal = Flag.CurrentOption end
				if CurrentVal == nil then CurrentVal = Flag.Color end

				if CurrentVal ~= FlagValue then
					changed = true
					Flag:Set(FlagValue)
				end
			end
			...
			Luna:Notification({
				Title = "Config Error",
				Content = "Luna was unable to load or find '" .. FlagName .. "' in the current script. Check " .. website .. " for help.",
				Icon = "flag"
			})
		end
	end

	if autoload and not notified then
		Luna:Notification({
...
local function SaveConfiguration(Configuration, ConfigFolder, hasRoot)
	...
	for i, v in pairs(Luna.Flags) do
		if v.Type == "ColorPicker" then
			...
			if val == nil then val = v.CurrentKeybind end
			if val == nil then val = v.CurrentOption end
			if val == nil then val = v.Color end
			Data[i] = val
		end
	end

	local path = BuildConfigPath(ConfigurationFolder, hasRoot, ConfigFolder, Configuration .. ConfigurationExtension)
	writefile(path, tostring(HttpService:JSONEncode(Data)))
end

local function SetAutoload(ConfigName, ConfigFolder, hasRoot)
	local path = BuildConfigPath(ConfigurationFolder, hasRoot, ConfigFolder, "autoload.txt")
	writefile(path, tostring(ConfigName) .. ConfigurationExtension)
end

local function LoadAutoLoad(ConfigFolder, hasRoot)
	local autoloadPath = BuildConfigPath(ConfigurationFolder, hasRoot, ConfigFolder, "autoload.txt")

	if isfile(autoloadPath) then
		local configFileName = readfile(autoloadPath) -- e.g. "MyConfig.json"
		local configPath = BuildConfigPath(ConfigurationFolder, hasRoot, ConfigFolder, configFileName)

		if isfile(configPath) then
			LoadConfiguration(readfile(configPath), true)
		end
	end
end
...
function Luna:Notification(data)
	task.spawn(function()
		...
		local resolvedIcon = GetIcon(data.Icon, data.ImageSource)
		if type(resolvedIcon) == "string" then
			newNotification.Icon.Image = resolvedIcon
		elseif type(resolvedIcon) == "table" and resolvedIcon.id then
			newNotification.Icon.Image = "rbxassetid://" .. resolvedIcon.id
			newNotification.Icon.ImageRectSize = resolvedIcon.imageRectSize
			newNotification.Icon.ImageRectOffset = resolvedIcon.imageRectOffset
		else
...
if UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled then
	MainSize = UDim2.fromOffset(Camera.ViewportSize.X, Camera.ViewportSize.Y)
	MinSize = UDim2.fromOffset(Camera.ViewportSize.X, 42)
...
function Luna:CreateWindow(WindowSettings)
	...
	if WindowSettings.ConfigSettings.Enabled then
		pcall(function()
			local BaseFolder = ConfigurationFolder
			...
			if RootFolder then
				local RootPath = BaseFolder .. "/" .. RootFolder
				if not isfolder(RootPath) then
					makefolder(RootPath)
				end

				local FullPath = RootPath .. "/" .. ConfigFolder
				if not isfolder(FullPath) then
					makefolder(FullPath)
				end
			else
				local FullPath = BaseFolder .. "/" .. ConfigFolder
				if not isfolder(FullPath) then
					makefolder(FullPath)
				end
			end

			LoadAutoLoad(ConfigFolder, RootFolder)
		end)
	end
	...
	function Window:CreateTab(TabSettings)
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						print("Flames Interface Suite | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									print("Flames Interface Suite | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Flames Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								...
								print("Flames Interface Suite | "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								print("Flames Interface Suite | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("Flames Interface Suite | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("Flames Interface Suite | "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact["MouseButton1Click"]:Connect(function()
				if Button:FindFirstChild("UIStroke") then tween(Button.UIStroke, {Color = Color3.fromRGB(136, 131, 163)}) end
				task.wait(0.2)
				local Success, Response = pcall(ButtonSettings.Callback)
				...
				if not Success then
					if Button.Parent then
						...
						if stroke then
							TweenService:Create(stroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
						else
							print("UIStroke not found inside Button: "..tostring(Button))
						end
						...
						stroke = Button:FindFirstChild("UIStroke")
						if stroke then
							TweenService:Create(stroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 0.5}):Play()
						else
							print("UIStroke not found inside Button: "..tostring(Button))
						end
					end
					...
					if ButtonV.Hover then
						tween(stroke, {Color = Color3.fromRGB(87, 84, 104)})
					else
						tween(stroke, {Color = Color3.fromRGB(64, 61, 76)})
					end
				end
			end)
			...
			Button["MouseEnter"]:Connect(function()
				ButtonV.Hover = true
				task.spawn(function() tween(Button.UIStroke, {Color = Color3.fromRGB(87, 84, 104)}) end)
			end)

			Button["MouseLeave"]:Connect(function()
				ButtonV.Hover = false
				task.spawn(function() tween(Button.UIStroke, {Color = Color3.fromRGB(64,61,76)}) end)
			end)
		...
		function Tab:CreateInput(InputSettings, Flag)
			...
			TweenService:Create(Input.InputFrame.InputBox, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {TextTransparency = 0}):Play()
			Input.InputFrame.InputBox.PlaceholderText = InputSettings.PlaceholderText
			local mobile_user = UserInputService.TouchEnabled
			local fixed_width = mobile_user and 220 or 160
			local fixed_height = mobile_user and 42 or 30
			Input.InputFrame.Size = UDim2.new(0, 100, 0, 30)
