...
g.toggle_rgb = g.toggle_rgb or function(name, state)
...
g.toggle_all_rgb = g.toggle_all_rgb or function(state)
...
g.flowrgb("InfPremiumScaledHolderRainbowRGB", 1, ScaledHolder, true)
g.flowrgb("InfinitePremiumHolderRainbowRGB", 1, Holder, true)
g.flowrgb("InfinitePCmdsFHolderRainbowRGB", 1, CMDsF, true)
g.flowrgb("TitleLabelRainbowRGB", 1, Title, true)
g.flowrgb("DarkLabelRainbowRGB", 1, Dark, true)
g.flowrgb("SettingsFrameRainbowRGB", 1, Settings, true)
shade1 = {}
shade2 = {}
shade3 = {}
text1 = {}
...
Title.TextSize = 18
Title.Text = "Infinite Premium FE v" .. currentVersion
...
PluginsHint.Text = "Download plugins from the Infinite Premium Discord (discord.gg/0)"
...
CMDs[#CMDs + 1] = {NAME = 'systembroken', DESC = 'Opens System Broken universal GUI.'}
CMDs[#CMDs + 1] = {NAME = 'netstats', DESC = 'Opens a GUI that shows Performance related information.'}
...
local function unseat(humanoid)
	...
	task.spawn(function()
		if humanoid.SeatPart then
			local seat_part = humanoid.SeatPart
			for _, child in pairs(seat_part:GetChildren()) do
				if child:IsA("Weld") or child:IsA("Motor6D") or child:IsA("WeldConstraint") then
					child:Destroy()
				end
			end
			local prox = seat_part:FindFirstChildOfClass("ProximityPrompt")
			if prox and prox.Enabled == false then
				prox.Enabled = true
			end
		end
		local jump_enabled = humanoid:GetStateEnabled(Enum.HumanoidStateType.Jumping) or false
...
addcmd('loopgoto', {}, function(args, speaker)
   ...
   for _, v in pairs(players) do
      ...
      if humanoid then
         speaker.Character:FindFirstChildOfClass('Humanoid').Sit = false
         speaker.Character:FindFirstChildWhichIsA("Humanoid"):ChangeState(3)
         wait()
      end
      ...
      repeat
         ...
         if targetPlayer and targetPlayer.Character then
            ...
            if speaker.Character:FindFirstChildOfClass("Humanoid").Sit or speaker.Character:FindFirstChildOfClass("Humanoid").Sit == true then
               speaker.Character:FindFirstChildOfClass('Humanoid').Sit = false
               speaker.Character:FindFirstChildWhichIsA("Humanoid"):ChangeState(3)
            end
