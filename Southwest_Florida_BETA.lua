...
getgenv().satisfied_color_tbl = getgenv().satisfied_color_tbl or {}
getgenv().scrolling_frame_obj = getgenv().scrolling_frame_obj or nil
task.wait(0.1)
if not getgenv().scrolling_frame_obj or not getgenv().scrolling_frame_obj:IsA("ScrollingFrame") then
    for _, v in ipairs(getgenv().PlayerGui:GetDescendants()) do
        if v:IsA("ScrollingFrame") and v:FindFirstChild("color1", true) then
            print("found scrollingframe: "..tostring(v.Name))
            getgenv().scrolling_frame_obj = v
            break
        end
    end
end
task.wait(0.5)
if getgenv().scrolling_frame_obj and getgenv().scrolling_frame_obj:IsA("ScrollingFrame") then
    if not next(getgenv().satisfied_color_tbl) then
        local seen = {}

        for _, v in ipairs(getgenv().scrolling_frame_obj:GetDescendants()) do
            if v:IsA("GuiObject") then
                ...
                if not (c.R == 1 and c.G == 1 and c.B == 1) then
                    ...
                    if not seen[key] then
                        seen[key] = true
                        table.insert(getgenv().satisfied_color_tbl, Color3.fromRGB(r, g, b))
                        print("adding color:", r, g, b)
                    end
                end
            end
        end
...
getgenv().get_FE_ColorPart = function(vehicle)
    ...
    for _,v in ipairs(vehicle:GetDescendants()) do
        ...
        if n:find("color") or n:find("body") or n:find("paint") then
            color_part = v
            break
        end
    end
